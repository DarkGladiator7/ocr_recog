from segmentation.image_segments_aligner import ImageSegmentsAligner
from segmentation.object_detection import ObjectDetection
import yolov9
import statistics
import torch
...
def extract_paragraphs(image, texts_b_boxes):
    ...
    for main_segment_info in main_segments:
        ...
        # paragraph_box = (seg_box[0], seg_box[1], seg_box[2]-seg_box[0], seg_box[3]-seg_box[1])
        ...
        sorted_text_boxes = list(sorted(zip(main_segment_info['token_bboxes'], main_segment_info['token_words']), key=lambda t: (t[1], t[0])))
