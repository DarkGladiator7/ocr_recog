...
import numpy as np
import time
from v6 import VisionLLM  # Importing your class
...
if st.session_state.image_uploaded and st.session_state.image_path:
    ...
    for text, (x, y, w, h), _ in paragraphs:
        ...
        detected_languages1.append(detected_lang_code)
    ...
    text_bboxes = ocr_translator.extract_text(image,detected_languages1)
    paragraphs = extract_paragraphs(image, texts_b_boxes=text_bboxes)
    print(f'{paragraphs}')
...
if st.session_state.image_uploaded:
    st.info(
        f"**Detected Language(s):** {', '.join(filter(None, st.session_state.detected_languages)) if st.session_state.detected_languages else 'N/A'}")
...
if st.session_state.selected_translation:
    ...
    for text, (x, y, w, h), _ in st.session_state.paragraphs:
        cropped_image = orig_image[y:y+h, x:x+w]
        detected_lang_code = ocr_translator.qwen(cropped_image)
        print(f"Lang Code - {detected_lang_code}\nText - {text}")
